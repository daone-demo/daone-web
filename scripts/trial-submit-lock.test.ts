...
test('试用申请提交使用独立锁、禁用按钮并传递幂等键', () => {
  ...
  assert.match(comboSrc, /ensureTrialIdempotencyKey\(/)
  assert.match(comboSrc, /clearTrialIdempotencyKey\(/)
  assert.match(
    comboSrc,
    /createTrialApplication\(\s*\{[\s\S]*?position:\s*trialPosition\.value\.trim\(\),[\s\S]*?\},\s*idempotencyKey,/,
  )
  assert.match(
    apiSrc,
  ...
  assert.match(comboSrc, /网络失败保留幂等键/)
})
